...
public sealed class NewFrontierMicrowaveSystem : EntitySystem
{
    ...
    {
        if (!component.Storage.ContainedEntities.Any() || HasComp<ActiveMicrowaveComponent>(uid) || !(TryComp<ApcPowerReceiverComponent>(uid, out var apc) && apc.Powered))
            return;
        ...
        foreach (var item in component.Storage.ContainedEntities.ToArray())
        {
            ...
            {
                solidID = _protoman.Index<StackPrototype>(stackComp.StackTypeId).Spawn;
        ...
        foreach (var item in component.Storage.ContainedEntities.ToArray())
        {
            ...
            if (ev.Handled)
            {
                _microwave.UpdateUserInterfaceState(uid, component);
                return;
            }
        ...
        activeComp.CookTimeRemaining = component.CurrentCookTimerTime * component.CookTimeMultiplier; // Frontier: CookTimeMultiplier<FinalCookTimeMultiplier
